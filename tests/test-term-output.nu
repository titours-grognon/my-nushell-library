...
let alert_result = do {nu -c "use ../nu-modules/term-output.nu ; term-output __alert 'ALERT'"} | complete
print $alert_result
...
let debug_result = do {nu -c "use ../nu-modules/term-output.nu ; term-output __debug 'DEBUG'"} | complete
print $debug_result
...
let error_result = do {nu -c "use ../nu-modules/term-output.nu ; term-output __error 'ERROR'"} | complete
print $error_result
...
let info_icon = "•"
let info_text = "INFO"
let info_result = do {nu -c "use ../nu-modules/term-output.nu ; term-output __info 'INFO'"} | complete
print $info_result
...
let success_result = do {nu -c "use ../nu-modules/term-output.nu ; term-output __success 'SUCCESS'"} | complete
...
let note_result = do {nu -c "use ../nu-modules/term-output.nu ; term-output __note 'You have a message ?'"} | complete
print $note_result
...
let timestamp_result = do {nu -c "use ../nu-modules/term-output.nu ; term-output __log-info 'Log format'"}
print $timestamp_result

assert ($timestamp_result | find --regex "[0-9]\{4\}-[01][0-9]-([0-2][0-9]|3[01]) ([01][0-9]|2[0-4]):[0-5][0-9]:[0-5][0-9]" | is-not-empty)
...
let separator_character_result = do {nu -c "use ../nu-modules/term-output.nu ; term-output __draw-line"} | complete
